...
router.post('/admin/create', authMiddleware, adminOnly, async (req, res) => {
  try {
    const { platform, title, description, requiresUpload, uploadLabel, videoUrl, linkLabel, credentialsFields } = req.body;
    ...
    const newTask = {
      ...
      credentialsFields: (Array.isArray(credentialsFields) && credentialsFields.length) ? credentialsFields : null,
      videoUrl: videoUrl || null, linkLabel: linkLabel || null, isCustom: true, createdAt: new Date().toISOString()
    };
